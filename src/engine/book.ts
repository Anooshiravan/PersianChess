...
import { NOMOVE } from './defs';
import { printGameLine } from './board';
import { SqFromAlg, ParseMove, SanityCheck } from './input';

export { SanityCheck };

export function LineMatch(BookLine: string, gameline: string): boolean {
...
export function BookMove(return_hint: boolean): any {
    const gameLine = printGameLine();
    const bookMoves: any[] = [];
    let lengthOfLineHack: number = gameLine.length;
    let hint: any = NOMOVE;
    ...
    for (let bookLineNum = 0; bookLineNum < brd_bookLines.length; ++bookLineNum) {
        if (LineMatch(brd_bookLines[bookLineNum], gameLine)) {
            let move: string = brd_bookLines[bookLineNum].substr(lengthOfLineHack + 1);
            if (move.indexOf(' ') !== -1) {
