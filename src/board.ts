/*
    ...
    Board Renderer: DOM rendering, board state, FEN parsing, pointer drag/drop
*/
...
// ═══════════════════════════════════════════════════
//  Constants — mirrors engine Defs.js exactly
// ═══════════════════════════════════════════════════
...
const _PCE_CHAR = '.PNWCBRSFQKpnwcbrsfqk';

// Frame squares — visually present but not playable
const FRAME_SQ: number[] = [
    28,
    29,
    30,
    31,
...
// ═══════════════════════════════════════════════════
//  Board mapping arrays (mirrors engine InitSq195To121 / InitFilesRanksBrd)
// ═══════════════════════════════════════════════════

const FilesBrd: number[] = new Array(BRD_SQ_NUM);
const RanksBrd: number[] = new Array(BRD_SQ_NUM);
const Sq121ToSq195: number[] = [];
const Sq195ToSq121: number[] = [];

function initSq121ToSq195(): void {
    for (let i = 0; i < 121; i++) Sq121ToSq195[i] = 195;
    for (let i = 0; i < BRD_SQ_NUM; i++) Sq195ToSq121[i] = 122;

    let file: number, rank: number, sq: number, sq121: number;
...
function initFilesRanksBrd(): void {
    for (let index = 0; index < BRD_SQ_NUM; ++index) {
        FilesBrd[index] = SQUARES.OFFBOARD;
        RanksBrd[index] = SQUARES.OFFBOARD;
    }
    const frameSet: { [key: number]: boolean } = {};
...
// ═══════════════════════════════════════════════════
//  Square coordinate helpers (mirrors engine InputOutput.js)
// ═══════════════════════════════════════════════════

export function fr2sq(f: number, r: number): number {
    return 27 + f + r * 13;
}
...
// PrSq: convert internal square to algebraic notation
...
// CBSQ2SQ: convert engine notation "g3" to internal square
export function CBSQ2SQ(cbsq: string): number {
...
// ═══════════════════════════════════════════════════
//  Grid layout — visual row/col ↔ square mapping
// ═══════════════════════════════════════════════════

let GRID_LAYOUT: number[][] = [];
let sqToGrid: { [sq: number]: { row: number; col: number } } = {};
let gridToSq: number[][] = [];

function buildGridLayout(): void {
    initSq121ToSq195();
    initFilesRanksBrd();
    GRID_LAYOUT = [];

    // Visual row 0 = engine rank 11 (top), visual row 10 = engine rank 1 (bottom)
    for (let r = 0; r < 11; r++) {
        const row: number[] = [];
        for (let c = 0; c < 11; c++) {
...
// ═══════════════════════════════════════════════════
//  Piece image cache & theme
// ═══════════════════════════════════════════════════

let _currentTheme = 'green';
const pieceImageCache: { [key: string]: HTMLImageElement } = {};

function getPiecePath(code: string): string {
    return `img/chesspieces/wikipedia/${code}.png`;
}

function preloadPieceImages(pieces: (string | null)[]): void {
...
interface BoardState {
    ...
    inCheck: number | null;
    inCheckmate: number | null;
    sideToMove: string;
    draggedFrom: number | null;
    dragGhost: HTMLDivElement | null;
}
...
function parseFenChar(ch: string): string | null {
...
function parseFen(fenStr: string): void {
    ...
    for (let i = 0; i < posStr.length; i++) {
        ...
        if (sq >= 0 && sq < 195 && sqToGrid[sq] !== undefined) {
...
function buildArrowOverlay(): void {
    if (!boardEl) return;
...
function cellCenterPct(sq: number): { x: number; y: number } | null {
...
function drawHintArrow(fromSq: number, toSq: number): void {
    if (!arrowSvg) return;
    clearArrows();
...
function renderBoard(): void {
    ...
    for (let row = 0; row < GRID_ROWS; row++) {
        for (let col = 0; col < GRID_COLS; col++) {
            ...
            // Centre square (f6, sq 97) gets a variant-specific overlay:
            // Persian → Princess crown; ASE → Egyptian Eye.
            if (sq === 97) {
                const gc = (window as unknown as { GameController?: { variant?: string } }).GameController;
...
function getCellForSq(sq: number | null): HTMLElement | null {
...
function updateStatus(): void {
    const cell = boardEl?.querySelector('[data-sq="102"]') as HTMLElement | null;
    if (!cell) return;
...
function onPointerDown(e: PointerEvent): void {
    ...
    const cell = (e.target as HTMLElement).closest('.playable');
    if (!cell) {
        dlog('pointerdown on non-playable cell');
        return;
    }

    const sq = parseInt((cell as HTMLElement).dataset.sq!, 10);
    const pieceCode = boardState.pieces[sq];
    ...
    // Determine if this is the player's piece to move
    const isWhite = pieceCode.charAt(0) === 'w';
    const playerColor =
        typeof (window as unknown as {
            GameController: { playerColor: string; engineOn: boolean; engineThinking: boolean };
        }) !== 'undefined'
            ? (
    ...
    boardState.dragGhost.style.width = `${String((cell as HTMLElement).offsetWidth)}px`;
    boardState.dragGhost.style.height = `${String((cell as HTMLElement).offsetHeight)}px`;
    boardState.dragGhost.style.left = `${String(e.clientX - (cell as HTMLElement).offsetWidth / 2)}px`;
    boardState.dragGhost.style.top = `${String(e.clientY - (cell as HTMLElement).offsetHeight / 2)}px`;
...
function onPointerUp(e: PointerEvent): void {
    ...
    if (target) {
        target = (target as HTMLElement).closest('.playable');
    }
    ...
    if (target) {
        const toSq = parseInt((target as HTMLElement).dataset.sq!, 10);
        ...
        if (toSq !== fromSq) {
            ...
            const gc = (window as unknown as { GameController: { engine: Worker | null } }).GameController;
            if (gc?.engine) {
...
// ═══════════════════════════════════════════════════
//  Board Operations (exported for app.js)
// ═══════════════════════════════════════════════════

function _setFen(fenStr: string): void {
    parseFen(fenStr);
}

function highlightMove(fromSq: number, toSq: number): void {
...
function setCheckHighlight(sq: number | null): void {
    boardState.inCheck = sq;
    boardState.inCheckmate = null;
    applyHighlights();
}

function setCheckmateHighlight(sq: number | null): void {
    boardState.inCheck = sq;
...
function flipBoard(): void {
    ...
    if (pieces) {
        for (let i = 0; i < pieces.length; i++) {
            (pieces[i] as HTMLElement).style.transform = boardState.flipped ? 'rotate(180deg)' : '';
        }
    }
}

function setBoardTheme(themeName: string): void {
...
function setVariantOverlay(variant: string): void {
    if (!boardEl) return;
    const centre = boardEl.querySelector('[data-sq="97"]') as HTMLElement | null;
    if (!centre) return;
...
function setEngineThinking(thinking: boolean): void {
    const cell = boardEl?.querySelector('[data-sq="102"]') as HTMLElement | null;
    if (!cell) return;
    if (thinking) {
...
// Export functions for use by app.ts
(
    window as unknown as {
        parseFen: typeof parseFen;
        highlightMove: typeof highlightMove;
        clearHighlights: typeof clearHighlights;
        setCheckHighlight: typeof setCheckHighlight;
        setCheckmateHighlight: typeof setCheckmateHighlight;
        flipBoard: typeof flipBoard;
        setBoardTheme: typeof setBoardTheme;
        setActive: typeof setActive;
        setEngineThinking: typeof setEngineThinking;
    }
).parseFen = parseFen;
(
    window as unknown as {
...
).setCheckmateHighlight = setCheckmateHighlight;
(
    window as unknown as {
        parseFen: typeof parseFen;
        highlightMove: typeof highlightMove;
...
).setEngineThinking = setEngineThinking;
(window as unknown as { setVariantOverlay: typeof setVariantOverlay }).setVariantOverlay = setVariantOverlay;
(window as unknown as { drawHintArrow: typeof drawHintArrow }).drawHintArrow = drawHintArrow;
(window as unknown as { clearArrows: typeof clearArrows }).clearArrows = clearArrows;

// Expose boardState for cross-bundle access (app.mjs needs it)
(window as unknown as { boardState: typeof boardState }).boardState = boardState;
